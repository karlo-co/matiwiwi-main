...
> El sitio usa `fetch()` para leer el JSON, por lo que **no funciona abriendo el
> archivo con doble clic** (protocolo `file://`). Debe ejecutarse en un servidor:

1. Abrir la carpeta en **VS Code**.
2. Instalar la extensión **Live Server**.
3. Clic derecho sobre `index.html` → **Open with Live Server**.
4. El navegador abrirá `http://127.0.0.1:5500/...` y los productos cargarán.

También funciona directamente desde la URL pública de GitHub Pages.

## Dificultades encontradas

- Los productos no se mostraban al abrir el archivo localmente; se resolvió al
  entender que `fetch()` requiere un servidor (`http://`) y no `file://`.
- Configuración de FormSubmit y del campo `_next` para redirigir a `gracias.html`.

## Pruebas realizadas

- Carga correcta de los productos desde el JSON en GitHub Pages.
- Cálculo del total al cambiar cantidades.
- Validaciones de ambos formularios (campos vacíos, correo inválido, sin productos,
  mensaje corto).
- Recepción de los correos de compra y de contacto.
- Visualización correcta en computador y celular.
